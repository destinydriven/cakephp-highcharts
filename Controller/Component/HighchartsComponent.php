...
class HighchartsComponent extends Component {
	...
	public function create($name, $type) {
		if (!isset($type)) {
			$this->log(sprintf(__('Please provide a type for chart %s!'), $name));
			return false;
		}
		$chart = null;
	...
	public function setChartParams($name, $params = array()) {
		...
		// chart options
		if (isset($params['chartAnimation'])) {
			$this->charts[$name]->chart->animation = $params['chartAnimation'];
		}
		...
		if (isset($params['tooltipEnabled']) && $params['tooltipEnabled'] === true) {
			...
			if (isset($params['tooltipPointFormat'])) {
				$this->charts[$name]->tooltip->pointFormat = new stdClass();
		...
		if (!isset($this->charts[$name]->plotOptions)) {
			$this->charts[$name]->plotOptions = new HighRollerPlotOptions($this->charts[$name]->chart->type);
		}
		// series
		if (!isset($this->charts[$name]->plotOptions->series)) {
			$this->charts[$name]->plotOptions->series = new HighRollerPlotOptions($this->charts[$name]->chart->type);
		}
		// column
		if (!isset($this->charts[$name]->plotOptions->column)) {
			$this->charts[$name]->plotOptions->column = new HighRollerPlotOptions($this->charts[$name]->chart->type);
		}
		// line
		if (!isset($this->charts[$name]->plotOptions->line)) {
			$this->charts[$name]->plotOptions->line = new HighRollerPlotOptions($this->charts[$name]->chart->type);
		}
		// area
		if (!isset($this->charts[$name]->plotOptions->area)) {
			$this->charts[$name]->plotOptions->area = new HighRollerPlotOptions($this->charts[$name]->chart->type);
		}
		// funnel
		if (!isset($this->charts[$name]->plotOptions->funnel)) {
			$this->charts[$name]->plotOptions->funnel = new HighRollerPlotOptions($this->charts[$name]->chart->type);
		}
		...
		if (isset($params['plotOptionsSeriesAnimation'])) {
			$this->charts[$name]->plotOptions->series->animation = $params['plotOptionsSeriesAnimation'];
		}
		...
		if (isset($params['plotOptionsLineDataLabelsFormat'])) {
			$this->charts[$name]->plotOptions->line->dataLabels = new HighRollerDataLabels();
			$this->charts[$name]->plotOptions->line->dataLabels->format = $params['plotOptionsLineDataLabelsFormat'];
		}
